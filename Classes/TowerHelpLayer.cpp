...
void TowerHelpLayer::PPageView()
{
	...
	for (int i = 0; i < 18; ++i)
	{
		...
		auto pLayoutSprite = Sprite::createWithSpriteFrameName(StringUtils::format("tower_%02d.png", i+1));
		auto pLayoutSpriteName = Sprite::createWithSpriteFrameName(StringUtils::format("tower_%02d_CN.png", i+1));
	...
	_pPageView->addEventListener([=](Ref *pSender, PageView::EventType event)
	{
		...
		{
			_iCurLevelIndex = _pPageView->getCurrentPageIndex();
