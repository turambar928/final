...
bool Carrot::init()
{
	...
	do
	{
		...
		_HpSprite = Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("BossHP10.png"));//建立精灵
		_HpSprite->setPosition(*(tPoints.end() - 1) - *(tPoints.end() - 2) + Vec2(-20,10));//位置
		addChild(_HpSprite);

		NOTIFY->addObserver(this,CC_CALLFUNCO_SELECTOR(Carrot::CarrotBeHurt), "CarrotHurt",nullptr);//注册一个观察者模式的回调函数
        registerCarrotTouchEvent();
        
		srand(time(0));
		schedule(CC_SCHEDULE_SELECTOR(Carrot::doAction), 6);//安排一个定时器，每隔 6 秒，会执行 Carrot::doAction 这个函数，让 Carrot 对象做一些动作。
