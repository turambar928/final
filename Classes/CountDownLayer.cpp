...
bool CountDownLayer::init(){
	...
	//倒计时的进度
	this->schedule(CC_SCHEDULE_SELECTOR(CountDownLayer::runArrow1), 0.40f);
	this->schedule(CC_SCHEDULE_SELECTOR(CountDownLayer::runArrow2), 0.60f);
	this->schedule(CC_SCHEDULE_SELECTOR(CountDownLayer::runArrow3), 0.80f);
	
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(CountDownLayer::Num1), 0.0f);
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(CountDownLayer::Num2), 1.0f);
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(CountDownLayer::Num3), 2.0f);
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(CountDownLayer::GameGo),3.0f);
	this->scheduleOnce(CC_SCHEDULE_SELECTOR(CountDownLayer::childDispear), 3.6f);
    registerNoTouch();
	return true;
}
